...
export default function Cashier({ products }) {
  ...
  const addToCart = (e) => {
    ...
    if (qty > pendingProduct.stock) return alert("Stok tidak mencukupi!")
  ...
  const checkout = () => {
    ...
    // Simpan data untuk struk sebelum cart direset
    ...
    router.post('/sales', { total_price: totalPrice, items: cart }, {
      onSuccess: () => {
    ...
    <div className="min-h-screen bg-[#0f172a] text-slate-200 p-4 md:p-8 font-sans">
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        @media print {
          ...
          #receipt-print { 
            width: 100%; 
            max-width: 80mm; /* Sesuaikan dengan lebar kertas Anda */
            padding: 2mm;
          ...
          /* Sembunyikan elemen lain secara total saat print */
      ...
      {/* MODAL STRUK (Hanya muncul saat showReceipt true) */}
      {showReceipt && lastTransaction && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-sm z-[60] flex items-center justify-center p-4 no-print">
          <div className="bg-white text-slate-900 w-full max-w-sm p-6 rounded-lg shadow-2xl overflow-hidden">
            ...
            <div id="receipt-print" className="font-mono text-sm">
              <div className="text-center mb-4 border-b border-dashed border-slate-300 pb-2">
                <h2 className="font-bold text-lg uppercase">TOKO AFUN</h2>
                ...
                {lastTransaction.items.map((item, i) => (
                  <div key={i} className="flex justify-between text-xs">
                    <div className="flex-1">
                      <div>{item.name}</div>
                      <div className="text-[10px]">{item.quantity} x {item.price.toLocaleString()}</div>
                    </div>
                    <div className="text-right">{(item.quantity * item.price).toLocaleString()}</div>
                  </div>
                ))}
              </div>
            ...
            {/* Tombol Aksi di Layar (Tidak ikut diprint) */}
            <div className="mt-6 flex gap-2">
              <button onClick={handlePrint} className="flex-1 bg-blue-600 text-white py-2 rounded font-bold text-sm">CETAK STRUK</button>
              <button onClick={closeReceipt} className="flex-1 bg-slate-200 text-slate-800 py-2 rounded font-bold text-sm">TUTUP</button>
            </div>
          </div>
        </div>
      )}

      {/* MODAL QUANTITY (Sama seperti sebelumnya) */}
      {pendingProduct && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div className="bg-[#1e293b] border border-blue-500 w-full max-w-md p-8 rounded-2xl shadow-2xl scale-100 animate-in zoom-in duration-150">
            <h3 className="text-blue-400 font-bold uppercase tracking-widest text-[10px] mb-2">Konfirmasi Item</h3>
            <h2 className="text-2xl font-bold mb-1 text-white">{pendingProduct.name}</h2>
            <p className="text-slate-400 text-sm mb-6 font-mono">Harga Satuan: Rp {pendingProduct.price.toLocaleString()}</p>
            
            <form onSubmit={addToCart}>
              <div className="mb-6">
                <label className="block text-slate-500 text-[10px] uppercase font-bold mb-2 text-center">Jumlah Beli</label>
                <input 
                  ref={qtyRef}
                  type="number" 
                  value={quantityInput}
                ...
                <div className="text-center mt-4 text-slate-300 text-lg">
                  Subtotal: <span className="text-emerald-400 font-bold">Rp {(pendingProduct.price * (parseInt(quantityInput) || 0)).toLocaleString()}</span>
                </div>
              </div>
              <div className="flex gap-3">
                <button type="button" onClick={() => setPendingProduct(null)} className="flex-1 py-4 bg-slate-700 hover:bg-slate-600 rounded-xl font-bold transition-all">BATAL</button>
                <button type="submit" className="flex-[2] py-4 bg-blue-600 hover:bg-blue-500 rounded-xl font-bold text-white shadow-lg transition-all uppercase tracking-widest">OK (Enter)</button>
              </div>
            </form>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-12 gap-6">
        {/* ... (Pencarian & Tabel Keranjang tetap sama) ... */}
        <div className="col-span-12 lg:col-span-8 flex flex-col gap-6">
          <div className="relative">
            <div className="bg-[#1e293b] p-6 rounded-2xl border border-slate-700/50 shadow-xl">
              <label className="block text-blue-400 text-[10px] font-black uppercase mb-2 tracking-[0.2em]">Pencarian Produk</label>
              <input 
                ref={searchRef}
                autoFocus
                type="text" 
            ...
            {searchResults.length > 0 && (
              <div className="absolute top-full left-0 right-0 mt-2 bg-[#1e293b] border border-slate-700 rounded-xl shadow-2xl z-40 max-h-60 overflow-y-auto">
                {searchResults.map((p) => (
                  <div key={p.id} onClick={() => selectProduct(p)} className="p-4 hover:bg-blue-600 cursor-pointer border-b border-slate-700 last:border-0 flex justify-between items-center group transition-colors">
                    <div>
                    ...
                    <div className="text-right">
                      ...
                      <div className="text-[10px] text-slate-500 group-hover:text-blue-100 font-bold uppercase tracking-tighter">Stok: {p.stock}</div>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="bg-[#1e293b] rounded-2xl border border-slate-700/50 flex-grow flex flex-col overflow-hidden shadow-xl">
            ...
            <div className="h-[420px] overflow-y-auto custom-scrollbar">
              {cart.length === 0 && (
                <div className="h-full flex items-center justify-center text-slate-600 italic text-sm">Keranjang Belanja Kosong</div>
              )}
              {cart.map((item, index) => (
                <div key={index} className="grid grid-cols-12 items-center p-4 border-b border-slate-800/50 hover:bg-slate-800/30 transition-all">
                  ...
                  <div className="col-span-2 flex justify-center">
                    <span className="bg-blue-500/10 text-blue-400 px-3 py-1 rounded-lg border border-blue-500/20 font-bold font-mono">{item.quantity}</span>
                  </div>
                  <div className="col-span-2 text-right font-black text-white">Rp {(item.price * item.quantity).toLocaleString()}</div>
                  <div className="col-span-1 text-right">
                    <button onClick={() => removeFromCart(item.product_id)} className="text-slate-700 hover:text-red-500 transition-colors p-1 text-lg">✕</button>
                  </div>
                </div>
              ))}
            </div>
  ...
  <div className="bg-[#1e293b] p-6 rounded-2xl border border-slate-700/50 shadow-xl flex flex-col">
    {/* flex-grow dihapus dari div di bawah ini agar tidak mendorong konten */}
    <div>
      <div className="mb-6 p-6 bg-slate-900/80 rounded-2xl border border-blue-500/30 ring-1 ring-blue-500/20">
        <span className="text-[10px] text-blue-400 block mb-2 uppercase font-black tracking-widest">Total Bayar</span>
      ...
      <div className="space-y-4">
        <div>
          <label className="block text-[10px] font-black text-slate-500 uppercase mb-2 tracking-widest pl-1">Tunai (Cash)</label>
          <div className="relative">
        ...
        {/* Jarak diatur di sini */}
        <div className="p-4 bg-slate-900/40 rounded-xl border border-slate-800/80 mb-4">
          <span className="block text-[10px] font-black text-slate-500 uppercase mb-1 tracking-widest">Kembalian</span>
    ...
    {/* mt-8 diubah menjadi mt-2 atau mt-4 agar lebih rapat */}
    <button 
      onClick={checkout}
