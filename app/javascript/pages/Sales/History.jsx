...
export default function History({ sales, filters }) {
  ...
  // Fungsi navigasi halaman (sama dengan Stock)
  const handlePageChange = (page) => {
    if (!page) return;
    router.get('/riwayat', { ...filters, page: page }, { 
      preserveState: true, 
      preserveScroll: true 
    });
  };
    ...
    const handleFilterChange = (e) => {
        router.get('/riwayat', { month: e.target.value }, { 
            preserveState: true, 
            replace: true 
        });
    };
  ...
  {data.length > 0 ? data.map((sale) => (
    <tr key={sale.id} className="hover:bg-slate-50 transition-colors">
      <td className="p-4 text-sm text-slate-600 font-mono">
        {/* Menampilkan waktu yang lebih detail agar terlihat urutannya */}
      ...
      <td className="p-4 text-sm font-medium text-slate-700">
        {sale.sale_items ? sale.sale_items.length : 0} Produk
      </td>
      ...
      <td className="p-4 text-right flex justify-end gap-3">
        ...
        <button
          onClick={() => {
            if (confirm('Hapus transaksi ini? Data stok tidak akan kembali otomatis.')) {
              // Pastikan URL ini sesuai dengan route 'destroy' di Rails Anda
    ...
    <tr>
      <td colSpan="4" className="p-12 text-center text-slate-400 italic">
        Data tidak ditemukan.
                        ...
                        {/* Taruh ini tepat di bawah penutup div .overflow-x-auto */}
                        <div className="bg-white border-t border-slate-100 px-6 py-4 flex items-center justify-between">
                        <div className="text-sm text-slate-500">
                            Menampilkan <span className="font-bold text-slate-800">{data.length}</span> dari <span className="font-bold text-slate-800">{total_count}</span> transaksi
                        </div>
                        ...
                        <div className="flex gap-2">
                            ...
                            className={`px-4 py-2 rounded-lg border font-bold text-xs transition-all ${
                                is_first_page ? 'text-slate-300 border-slate-100' : 'text-blue-600 border-blue-200 hover:bg-blue-50'
                            }`}
                            >
                            SEBELUMNYA
                            </button>
                            ...
                            className={`px-4 py-2 rounded-lg border font-bold text-xs transition-all ${
                                is_last_page ? 'text-slate-300 border-slate-100' : 'text-blue-600 border-blue-200 hover:bg-blue-50'
                            }`}
                            >
                            SELANJUTNYA
                            </button>
